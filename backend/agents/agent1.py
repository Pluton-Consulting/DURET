"""
Agent 1 — Administratif / Suivi de chantiers (BTP — Duret & Sols)
...
logger = logging.getLogger("duret.agents.agent1")
...
DOSSIERS LONGS : les pièces sont conservées intégralement dans le dossier de ce fil. Pour rédiger un document fondé sur plusieurs pièces, utilise composer_document_dossier ; il lit, rédige, reprend et contrôle par sections. Ajoute les références NAS/Drive avec ajouter_source_dossier lorsque la recherche les retrouve. Pour les quantités et métrés, utilise produire_quantitatif après lecture des CCTP et des plans ; analyser_plan_source lit aussi un plan retrouvé au stockage. Ne remplace jamais un quantitatif par un inventaire, ni une création par l’ouverture d’un source. Reprends une tâche documentaire partielle avec son identifiant, sans demander de reformuler. Quand la demande NOMME un dossier du serveur (« à partir du dossier X », « les métrés du projet X »), passe ce nom tel quel dans `dossier` de produire_quantitatif ou de composer_document_dossier : ses fichiers utiles (lots visés, pièces communes, plans) sont chargés d'office, sous-dossiers compris — ne les ajoute pas un par un et ne demande pas qu'on te les joigne.
DOCUMENTS DE LA MAISON : avant de produire un nouveau devis, rapport, courrier ou mémoire, utilise preparer_document_maison pour retrouver la référence choisie et les trames disponibles. Si un fichier de référence est déjà fourni, inspecte-le directement avec reproduire_document. Réutilise le fichier original par utiliser_trame ou reproduire_document lorsqu’il convient : une reconstruction générique ne conserve pas sa présentation. Lis les références trouvées et les images avant de les annoncer comme reprises. Ne demande pas à la personne un fichier que les outils peuvent déjà retrouver.
DEUX FAÇONS DE REPRENDRE UN MODÈLE, À NE PAS CONFONDRE : (1) changer QUELQUES TEXTES d'un document existant (un client, une date, un montant) = reproduire_document ou utiliser_trame avec `remplacements` ; (2) REMPLIR le modèle de la maison avec un contenu NOUVEAU rédigé depuis des pièces (mémoire technique, rapport, dossier de réponse) = composer_document_dossier, avec `trame` = le nom de la trame enregistrée quand la personne en désigne une (« utilise le mémoire technique type ») : la page de garde, les en-têtes et les rubriques d'entreprise du modèle sont gardés, les rubriques du projet sont rédigées dans ses styles. Ne réponds jamais à une demande de type (2) par quelques remplacements sur la page de garde : le corps resterait celui de l'ancien chantier. Pour poser l'en-tête ou le pied de page de la maison sur un document DÉJÀ terminé : habiller_document (ajouter_document ne règle la présentation que d'un document ouvert).
...
Tu disposes d'une mémoire d'entreprise : fichiers importés (clients, devis, factures, fournisseurs), documents (appels d'offres, CCTP, CCAP, DPGF, plans, DOE, commandes, situations, documents fournisseurs, dossiers juridiques, réserves, dossiers chantier), mails.
...
async def routeur_node(state: AgentState) -> dict:
    ...
    invite = (
        ...
        "Dis UNIQUEMENT s'il faut consulter la mémoire d'entreprise (chantiers, devis, "
        "factures, commandes, dossiers, mails, documents internes) pour répondre.\n"
...
async def llm_node(state: AgentState, config=None) -> dict:
    ...
    if _anciens:
        ...
        # plus courte est périmé, il n'écrase plus les corrections (audit D-13).
    ...
    # du 1er tour (la mémoire semblerait perdue par intermittence), et deux utilisateurs
    # posant la même question partageraient une réponse conditionnée par la conversation
    # de l'autre. Le thread_id isole par conversation, donc par utilisateur.
    ...
    #
    # ET LES TERMINÉS AUSSI. Relevé en production : « test 2 » venait d'être
    # finalisé (38 Ko), et au tour suivant le modèle a répondu « ce document
    # n'existe pas », est parti le chercher sur le serveur, puis a proposé de
    # déposer un document VIDE à sa place. Un document fini sortait de la seule
    # liste que le modèle voyait — le plus important devenait invisible.
    try:
    ...
    if en_cours or finis:
        ...
        if finis:
            etat_docs += (
                "- TERMINÉS, fichier PRÊT et téléchargeable, mais PAS déposé "
                "sur le serveur :\n"
                + _json_docs.dumps(finis, ensure_ascii=False)
                + "\n  Ces documents ne se trouvent PAS en cherchant sur le "
                  "serveur : ils vivent ici, avec leur `document_id`. Titre, "
...
async def tools_node(state: AgentState, config=None) -> dict:
    ...
    if erreur:
        ...
        # à l'infini.
        #
        # DEUX PLUTÔT QU'UNE. Relevé en production le 14/08 : deux sorties
        # coupées d'affilée sur le même tour (le modèle a réessayé aussi long).
        # La première armait la reprise, la seconde terminait le tour — après
        # dix minutes de rédaction, sans rien avoir versé. Corriger un volume
        # demande parfois deux essais ; chacun coûte un appel, et le plafond
        # reste franc.
    ...
    # abandonner LIBÈRE une place, créer la reprend, et le cycle
    # abandon → création → remplissage → abandon tourne sans jamais croiser de
    # plafond. Huit minutes, deux fois le travail détruit. Un geste qui DÉFAIT
    # n'est jamais un geste qui avance.
...
async def validation_check_node(state: AgentState) -> dict:
    """Détecte si la réponse nécessite une validation humaine.
...
async def forcer_action_node(state: AgentState, config=None) -> dict:
    ...
    if finis:
        demande += (
            "\n\nDocument(s) TERMINÉS (fichier prêt, pas déposé) :\n"
            + _json.dumps(finis, ensure_ascii=False)
            + "\nSi l'intention est de DÉPOSER ou de montrer un de ces "
...
async def verifier_node(state: AgentState, config=None) -> dict:
    ...
    resume_resultats = "\n".join(
        ...
        # Le relecteur doit disposer des mêmes preuves que le rédacteur. La
        # réduction à 1 500 caractères le faisait nier 144 mails réellement lus.
