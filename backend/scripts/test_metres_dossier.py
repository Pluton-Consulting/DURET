...
from ressources import activite as act
importlib.reload(act)
async def _scenario():
    await act.dire(uid, fil2, "tache-ecran", "rédaction de la rubrique « Méthodologie »")
    await act.preciser("demande envoyée au modèle — environ 12 page(s) de pièces à lire")
    async def parallele():
        await act.dire(uid, fil2, "tache-ecran", "rédaction de la rubrique « Planning »")
    await asyncio.gather(parallele())
    await act.preciser("le modèle principal n’a pas répondu dans son délai : le modèle de secours prend le relais")
asyncio.run(_scenario())
brut = dossiers.etape(uid, fil2, "tache-ecran", "activite")
texte = act.phrase(brut, maintenant=__import__("time").time() + 75)
verifier("pendant un appel long, l'écran dit ce qui arrive (secours) et depuis combien de temps", "Méthodologie" in texte and "modèle de secours prend le relais" in texte and "depuis 1 min 15" in texte, texte)
...
verifier("les étapes longues du mémoire parlent toutes (accès, corrections, contrôles, mise en page)", composeur_a.count("await dire(uid,fil,tache,") >= 12, str(composeur_a.count("await dire(uid,fil,tache,")))
